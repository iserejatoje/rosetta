...
class Client
{
    ...
    private $_images_dir = '/common_fs/i/common_clients/';
    private $_images_url = '/resources/fs/i/common_clients/';
    ...
    function __construct(array $info)
    {
        ...
        if ( isset($info['clientid']) && Data::Is_Number($info['clientid']) )
            $this->_values["clientid"] = $info['clientid'];
        else
            $this->_values["clientid"] = 0;
    ...
    */
    public function Upload($inputName, $name)
    {
        ...
        $pname = FileStore::Upload_NEW(
            $inputName, $this->_images_dir, rand(1000, 9999).$this->_values['clientid'].'_'.mb_strtolower($name),
    ...
    {
        if ($this->_values['clientid'] === 0)
        {
            return  $this->_values["clientid"] = ClientMgr::getInstance()->AddClient($this->_values);
        }
        else
        {
            return ClientMgr::getInstance()->UpdateClient($this->_values);
        }
    }
...
//  Удаление продуктов
    public function Remove()
    {
        ...
        return ClientMgr::getInstance()->RemoveClient($this->_values['clientid']);
    }
