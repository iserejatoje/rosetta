...
class Question
{
	...
	private $_images_dir = '/common_fs/i/common_faq/';
	private $_images_url = '/resources/fs/i/common_faq/';
	...
	function __construct(array $info)
	{
		...
		if ( isset($info['questionid']) && Data::Is_Number($info['questionid']) )
			$this->_values["questionid"] = $info['questionid'];
		else
			$this->_values["questionid"] = 0;
	...
	public function __get($name)
	{
		...
		if ($name == 'id')
			return $this->_values['questionid'];
	...
	*/
	public function Upload($inputName, $name)
	{
		...
		$pname = FileStore::Upload_NEW(
			$inputName, $this->_images_dir, rand(1000, 9999).$this->_values['questionid'].'_'.mb_strtolower($name),
	...
	{
		if ($this->_values['questionid'] === 0)
		{
			return  $this->_values["questionid"] = FaqMgr::getInstance()->AddQuestion($this->_values);
		}
		else
		{
			return FaqMgr::getInstance()->UpdateQuestion($this->_values);
		}
	}
...
//  Удаление продуктов
	public function Remove()
	{
		...
		return FaqMgr::getInstance()->RemoveQuestion($this->_values['questionid']);
	}
