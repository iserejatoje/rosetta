...
class NSTreeMgr implements IDisposable {
	...
	private $_fields		= array('nodeid', 'left','right','level','parent','treeid','siteid','regionid','childscount', 'nameid');
	...
	public function setTreeId($treeId, $return_root = true) {
		...
		if ($info === false) {
			...
			if ($this->IsCache())
				$this->_cache->set($cacheid, $info, 600);
		}
	...
	public function getNode($nodeId) {
		...
		$node = false;
		$cacheid = 'node_'.(int) $nodeId;
		if ($this->IsCache())
			$node = $this->_cache->get($cacheid);
		...
		if ($node === false) {
			...
			if ($this->IsCache())
				$this->_cache->set($cacheid, $node, 3600);
		}
