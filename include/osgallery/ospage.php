...
class OSPage
{
    ...
    private $_images_dir = '/common_fs/i/common_ospage/';
    private $_images_url = '/resources/fs/i/common_ospage/';
    ...
    function __construct(array $info)
    {
        ...
        if ( isset($info['pageid']) && Data::Is_Number($info['pageid']) )
            $this->_values["pageid"] = $info['pageid'];
        else
            $this->_values["pageid"] = 0;
    ...
    */
    public function Upload($inputName, $name)
    {
        ...
        $pname = FileStore::Upload_NEW(
            $inputName, $this->_images_dir, rand(1000, 9999).$this->_values['pageid'].'_'.mb_strtolower($name),
    ...
    {
        if ($this->_values['pageid'] === 0)
        {
            return  $this->_values["pageid"] = OSGalleryMgr::getInstance()->AddPage($this->_values);
        }
        else
        {
            return OSGalleryMgr::getInstance()->UpdatePage($this->_values);
        }
    }
...
//  Удаление продуктов
    public function Remove()
    {
        ...
        return OSGalleryMgr::getInstance()->RemovePage($this->_values['pageid']);
    }
