...
class Location
{
	...
	private static $_config = array(
		...
		'_images_url' => '/_CDN/_i/location/1/',
	...
	*/
	...
	public static function DeleteBuild($BuildID)
	{
		...
		foreach($list as $Picture)
		{
			if ( !empty($Picture['ImageSmall']) )
			{
				try
				{
					FileStore::Delete_NEW(self::$_config['_images_dir'] . FileStore::GetPath_NEW($Picture['ImageSmall']['path']));
				}
				catch ( MyException $e ) {}
			}
			...
			if ( !empty($Picture['ImageBig']) )
			{
				try
				{
					FileStore::Delete_NEW(self::$_config['_images_dir'] . FileStore::GetPath_NEW($Picture['ImageBig']['path']));
				}
				catch ( MyException $e ) {}
			}
	...
	public static function AddBuildPicture($BuildID, $Name, array $info)
	{
		...
		$big_pname = FileStore::GetPath_NEW($big_pname);
		$big_pname = Images::PrepareImageToObject($big_pname, self::$_config['_images_dir'], self::$_config['_images_url']);
		$big_pname = FileStore::ObjectToString($big_pname);

		$small_pname = FileStore::GetPath_NEW($small_pname);
		$small_pname = Images::PrepareImageToObject($small_pname, self::$_config['_images_dir'], self::$_config['_images_url']);
		$small_pname = FileStore::ObjectToString($small_pname);
	...
	public static function EditBuildPicture($PictureID, $Name, array $info)
	{
		...
		if (isset($_FILES[$Name]))
		{
			...
			$Picture['ImageBig'] = FileStore::ObjectToString($big_pname);
			$Picture['ImageSmall'] = FileStore::ObjectToString($small_pname);
		}
	...
	public static function GetBuildingsPicturesList($filter) {
		...
		while (false != ($row = $res->fetch_assoc()))
		{
			if ( !empty($row['ImageSmall']) )
			{
				try
				{
					$img_obj = FileStore::ObjectFromString($row['ImageSmall']);
					$img_obj['file'] = FileStore::GetPath_NEW($img_obj['file']);
					$row['ImageSmall'] = Images::PrepareImageFromObject($img_obj, self::$_config['_images_dir'], self::$_config['_images_url']);
					unset($img_obj);
				}
				catch ( MyException $e )
				{
			...
			if ( !empty($row['ImageBig']) )
			{
				try
				{
					$img_obj = FileStore::ObjectFromString($row['ImageBig']);
					$img_obj['file'] = FileStore::GetPath_NEW($img_obj['file']);
					$row['ImageBig'] = Images::PrepareImageFromObject($img_obj, self::$_config['_images_dir'], self::$_config['_images_url']);
					unset($img_obj);
				}
				catch ( MyException $e )
				{
	...
	*/
	public static function AddLandmark($parent,$Name)
	{
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	private static function __cache_init()
	{
		if ( self::$_cache === null )
		{
			...
			self::$_cache->Init('memcache', 'location');
		}
	}
