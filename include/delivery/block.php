...
class Block
{
	...
	private $_images_dir = '/common_fs/i/common_block/';
	private $_images_url = '/resources/fs/i/common_block/';
	...
	function __construct(array $info)
	{
		...
		if ( isset($info['blockid']) && Data::Is_Number($info['blockid']) )
			$this->_values["blockid"] = $info['blockid'];
		else
			$this->_values["blockid"] = 0;
	...
	*/
	public function Upload($inputName, $name)
	{
		...
		$pname = FileStore::Upload_NEW(
			$inputName, $this->_images_dir, rand(1000, 9999).$this->_values['blockid'].'_'.mb_strtolower($name),
	...
	{
		if ($this->_values['blockid'] === 0)
		{
			return  $this->_values["blockid"] = DeliveryMgr::getInstance()->AddBlock($this->_values);
		}
		else
		{
			return DeliveryMgr::getInstance()->UpdateBlock($this->_values);
		}
	}
...
//  Удаление продуктов
	public function Remove()
	{
		...
		return DeliveryMgr::getInstance()->RemoveBlock($this->_values['blockid']);
	}
