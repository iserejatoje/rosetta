...
class Vacancy
{
	...
	private $_images_dir = '/common_fs/i/common_vacancy/';
	private $_images_url = '/resources/fs/i/common_vacancy/';
	...
	function __construct(array $info)
	{
		...
		if ( isset($info['vacancyid']) && Data::Is_Number($info['vacancyid']) )
			$this->_values["vacancyid"] = $info['vacancyid'];
		else
			$this->_values["vacancyid"] = 0;
	...
	*/
	public function Upload($inputName, $name)
	{
		...
		$pname = FileStore::Upload_NEW(
			$inputName, $this->_images_dir, rand(1000, 9999).$this->_values['vacancyid'].'_'.mb_strtolower($name),
	...
	{
		if ($this->_values['vacancyid'] === 0)
		{
			return  $this->_values["vacancyid"] = VacancyMgr::getInstance()->AddVacancy($this->_values);
		}
		else
		{
			return VacancyMgr::getInstance()->UpdateVacancy($this->_values);
		}
	}
...
//  Удаление продуктов
	public function Remove()
	{
		...
		return VacancyMgr::getInstance()->RemoveVacancy($this->_values['vacancyid']);
	}
