...
#		'X-Mailer',
	...
	{
		// эта проверка сделана изза провайдеров авторизации, которые не предоставляют данные о email пользователя
		// у на генерируются подобные адреса - 164955884@facebook.gilmon.ru
		// на такие адреса отправлять безсмысленно
		if (preg_match("@\@(openid|google|facebook|twitter|live|linkedin|vkontakte|yandex|mailru|lastfm|instagram|vimeo|tumblr)\.gilmon\.ru@", $to))
			return true;
		
		if ($this->is_blacklist($to))
			return true;
		
		if(@mail($to, $subject, $body, $header, "-f".$this->from_email) )
		{
	...
	private function is_blacklist($email)
	{
		$db = DBFactory::GetInstance($this->__config['db']);

		$sql = "SELECT count(*) FROM ".$this->__config['tables']['black_list'];
	...
	private function __generate_body()
	{
		...
		if(count($this->_body)==1)
		{
			foreach($this->_body as $k=>$v)
			{
				...
				$this->__body = chunk_split(base64_encode($v['value']), 76, "\n");
			}
		}
...
#			$this->__add_header('X-Mailer', 'WebMail (v1.00)');
	...
	public function SendDelayed($body = true, $header = true)
	{
		return $this->__send($body, $header, false);
	}
