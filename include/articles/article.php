<?php

LibFactory::GetStatic('arrays');

class Article
{
	private $_mgr;

	public $ID;
	public $SectionID;
	public $Created;
	public $LastUpdated;
	private $Date;
	public $ShortTitle;
	public $Title;
	public $AnnounceText;
	public $Text;
	public $SourceLink;
	public $IsVisible;
	public $IsComments;
	public $IsRSS;
	public $IsMain;
	public $IsImportant;
	public $Views;
	public $CountComments;
	public $SeoTitle;
	public $SeoDescription;
	public $SeoKeywords;

	private $Thumb;
	private $Photo;

	private $_cachePhotos = null;
	private $_cacheUrl = null;

	private $_images_dir = '/common_fs/i/articles/';
	private $_images_url = '/resources/fs/i/articles/';

	private $_files_dir = '/common_fs/f/articles/';
	private $_files_url = '/resources/fs/f/articles/';

	private $thumb_size = array('max_width' => 94, 'max_height' => 88);
	private $photo_size = array('max_width' => 392, 'max_height' => 246);

	private $photo_file_size = 716800; //700K
	private $file_size = 2097152; // 2M

	function __construct(array $info)
	{
		global $OBJECTS;

		$info = array_change_key_case($info, CASE_LOWER);

		if ( isset($info['articleid']) && Data::Is_Number($info['articleid']) )
			$this->ID = $info['articleid'];

		$this->SectionID		= (int) $info['sectionid'];
		$this->Created			= $info['created'];
		$this->LastUpdated		= $info['lastupdated'];
		$this->Date				= $info['date'];
		$this->ShortTitle		= stripslashes($info['shorttitle']);
		$this->Title			= stripslashes($info['title']);
		$this->AnnounceText		= stripslashes($info['announcetext']);
		$this->Text				= stripslashes($info['text']);
		$this->SourceLink		= stripslashes($info['sourcelink']);
		$this->IsVisible		= $info['isvisible'] ? true : false;
		$this->IsComments		= $info['iscomments'] ? true : false;
		$this->IsRSS			= $info['isrss'] ? true : false;
		$this->IsMain			= $info['ismain'] ? true : false;
		$this->IsImportant		= $info['isimportant'] ? true : false;
		$this->Views			= (int) $info['views'];
		$this->CountComments	= (int) $info['countcomments'];
		$this->Thumb			= $info['thumb'];
		$this->Photo			= $info['photo'];
		$this->SeoTitle			= stripslashes($info['seotitle']);
		$this->SeoDescription	= stripslashes($info['seodescription']);
		$this->SeoKeywords		= stripslashes($info['seokeywords']);
	}


	/**
	 * сохранить информацию в базе
	 * использует метод плагина и метод ArticleMgr
	 */
	public function Update()
	{
		$info = array(
            'SectionID'        	=> (int) $this->SectionID,
            'Date'            	=> $this->Date,
			'ShortTitle'		=> stripslashes($this->ShortTitle),
			'Title'				=> stripslashes($this->Title),
			'AnnounceText'		=> stripslashes($this->AnnounceText),
			'Text'				=> stripslashes($this->Text),
			'SourceLink'		=> stripslashes($this->SourceLink),
			'IsVisible'			=> (int) $this->IsVisible,
			'IsComments'		=> (int) $this->IsComments,
			'IsRSS'				=> (int) $this->IsRSS,
			'IsMain'			=> (int) $this->IsMain,
			'IsImportant'		=> (int) $this->IsImportant,
			'Views'				=> (int) $this->Views,
			'CountComments'		=> (int) $this->CountComments,
			'Thumb'				=> $this->Thumb,
			'Photo'				=> $this->Photo,
			'SeoTitle'			=> stripslashes($this->SeoTitle),
			'SeoDescription'	=> stripslashes($this->SeoDescription),
			'SeoKeywords'		=> stripslashes($this->SeoKeywords),
		);

		if ( $this->ID !== null )
		{
			$info['ArticleID'] = $this->ID;
			if ( false !== ArticleMgr::getInstance()->Update($info))
			{
				return true;
			}
		}
		else if ( false !== ($new_id = ArticleMgr::getInstance()->Add($info)))
		{
			$this->ID = $new_id;
			return $new_id;
		}

		return false;
	}

	public function UpdateViews()
	{
		ArticleMgr::getInstance()->UpdateViews($this->ID);
	}

	public function UpdateCountComments($plus_minus)
	{
		if (!in_array($plus_minus, array('plus','minus')))
			return;
		ArticleMgr::getInstance()->UpdateCountComments($plus_minus, $this->ID);
	}

	public function Remove()
	{
		if ( $this->ID === null)
			return false;

		ArticleMgr::getInstance()->Remove($this->ID);
	}


	private function _deleteThumb()
	{
		try
		{
			if( ($img_obj = FileStore::ObjectFromString($this->Thumb)) !== false )
			{
				$del_file = $this->_images_dir.FileStore::GetPath_NEW($img_obj['file']);
				if (FileStore::IsFile($del_file))
					return FileStore::Delete_NEW($del_file);
			}
		}
		catch(MyException $e) { }
		return false;
	}

	public function UploadThumb()
	{

		global $OBJECTS;
		LibFactory::GetStatic('filestore');
		LibFactory::GetStatic('filemagic');

		if (empty($_FILES['Thumb']['name']))
			return "";

		$pname = FileStore::Upload_NEW(
			'Thumb', $this->_images_dir, rand(1000, 9999).$this->ID.'_thumb',
			FileMagic::MT_WIMAGE, $this->photo_file_size,
			array(
				'resize' => array(
					'tr' => 3,
					'w'  => $this->thumb_size['max_width'],
					'h'  => $this->thumb_size['max_height'],
				),
			)
		);

		$pname = FileStore::GetPath_NEW($pname);
		$thumbNew = Images::PrepareImageToObject($pname, $this->_images_dir);
		$pname = FileStore::ObjectToString($thumbNew);

		$this->__set('Thumb', $pname);
		return $result;
	}

	private function _deletePhoto()
	{
		try
		{
			if( ($img_obj = FileStore::ObjectFromString($this->Photo)) !== false )
			{
				$del_file = $this->_images_dir.FileStore::GetPath_NEW($img_obj['file']);
				if (FileStore::IsFile($del_file))
					return FileStore::Delete_NEW($del_file);
			}
		}
		catch(MyException $e) { }
		return false;
	}

	public function UploadPhoto()
	{

		global $OBJECTS;
		LibFactory::GetStatic('filestore');
		LibFactory::GetStatic('filemagic');

		if (empty($_FILES['Photo']['name']))
			return "";

		$pname = FileStore::Upload_NEW(
			'Photo', $this->_images_dir, rand(1000, 9999).$this->ID.'_photo',
			FileMagic::MT_WIMAGE, $this->photo_file_size,
			array(
				'resize' => array(
					'tr' => 3,
					'w'  => $this->photo_size['max_width'],
					'h'  => $this->photo_size['max_height'],
				),
			)
		);

		$pname = FileStore::GetPath_NEW($pname);
		$photoNew = Images::PrepareImageToObject($pname, $this->_images_dir);
		$pname = FileStore::ObjectToString($photoNew);

		$this->__set('Photo', $pname);
		return $result;
	}

	public function __set($name, $value)
	{
		LibFactory::GetStatic('images');
        LibFactory::GetStatic('filestore');

		$name = strtolower($name);
		switch($name)
		{
			case 'thumb':
				if ($value === null)
				{
					if ($this->Thumb)
						$this->_deleteThumb($this->Thumb);

					$this->Thumb = '';
					return $value;
				}

				try
				{
					if( ($img_obj = FileStore::ObjectFromString($value)) !== false )
					{
						$file = $this->_images_dir.FileStore::GetPath_NEW($img_obj['file']);
						if (FileStore::IsFile($file)) {
							$this->__set($name, null);
							return $this->Thumb = FileStore::ObjectToString($img_obj);
						}
					}
				}
				catch(MyException $e) { }
			break;

			case 'photo':
				if ($value === null)
				{
					if ($this->Photo)
						$this->_deletePhoto($this->Photo);

					$this->Photo = '';
					return $value;
				}

				try
				{
					if( ($img_obj = FileStore::ObjectFromString($value)) !== false )
					{
						$file = $this->_images_dir.FileStore::GetPath_NEW($img_obj['file']);
						if (FileStore::IsFile($file)) {
							$this->__set($name, null);
							return $this->Photo = FileStore::ObjectToString($img_obj);
						}
					}
				}
				catch(MyException $e) { }
			break;

			case 'date':
				$this->Date = date('Y-m-d H:i:s', $value);
			break;
		}
		return null;
	}

	public function __get($name)
	{
		$name = strtolower($name);
		switch($name)
		{
			case 'thumb':
				if (!$this->Thumb)
					return null;

				LibFactory::GetStatic('images');
				LibFactory::GetStatic('filestore');

				try
				{
					$img_obj = FileStore::ObjectFromString($this->Thumb);
					$img_obj['file'] = FileStore::GetPath_NEW($img_obj['file']);
					$preparedImage = Images::PrepareImageFromObject($img_obj,
						$this->_images_dir, $this->_images_url);
					unset($img_obj);
					if (empty($preparedImage))
						return null;
				}
				catch ( MyException $e )
				{
					return null;
				}

				return array(
					'f' => $preparedImage['url'],
					'w' => $preparedImage['w'],
					'h' => $preparedImage['h'],
				);
			case 'photos':
				if ($this->_cachePhotos !== null)
					return $this->_cachePhotos;

				$filter = array(
					'flags' => array(
						'objects' => true,
						'PlaceID' => $this->ID,
					),
					'field' => array(
						'Ord',
					),
					'dir' => array(
						'ASC',
					),
				);

				$this->_cachePhotos = ArticleMgr::getInstance()->GetPhotos($filter);
				return $this->_cachePhotos;
			case 'url':
				if ($this->_cacheUrl !== null)
					return $this->_cacheUrl;
				$this->_cacheUrl = ModuleFactory::GetLinkBySectionId($this->SectionID).$this->ID."/";
				return $this->_cacheUrl;

			case 'date':
				return strtotime($this->Date);
			break;
			case 'photo':
				if (!$this->Photo)
					return null;

				LibFactory::GetStatic('images');
				LibFactory::GetStatic('filestore');

				try
				{
					$img_obj = FileStore::ObjectFromString($this->Photo);
					$img_obj['file'] = FileStore::GetPath_NEW($img_obj['file']);
					$preparedImage = Images::PrepareImageFromObject($img_obj,
						$this->_images_dir, $this->_images_url);
					unset($img_obj);
					if (empty($preparedImage))
						return null;
				}
				catch ( MyException $e )
				{
					return null;
				}

				return array(
					'f' => $preparedImage['url'],
					'w' => $preparedImage['w'],
					'h' => $preparedImage['h'],
				);
		}

		return null;
	}

	function __destruct()
	{

	}
}