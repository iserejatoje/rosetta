...
require_once ($CONFIG['engine_path'].'include/articles/article.php');
...
class ArticleMgr
{
	...
	public function getCache()
	{
		...
		$cache = new Cache();
		$cache->Init('memcache', 'articlemgr');
	...
	{
		$sql = "SELECT ArticleID FROM ".$this->_tables['articles'];
	...
	{
		$sql  = "SELECT DISTINCT YEAR(Date) year FROM ".$this->_tables['articles'];
		$sql .= " WHERE SectionID = ". App::$Env['sectionid'];
	...
	public function GetArticles($filter)
	{
		...
		if ( $filter['flags']['Year'] != -1 )
			$where[] = ' YEAR('.$this->_tables['articles'].'.Date) = '.$filter['flags']['Year'];
		...
		if (isset($filter['flags']['Period']) && is_array($filter['flags']['Period']))
		{
			...
			$d = implode('-', $date);
			$where[] = " ".$this->_tables['articles'].".`Date` >= '".$d."'";

			if ( $filter['flags']['Period']['day'] )
				$where[] = " ".$this->_tables['articles'].".`Date` < DATE_ADD('".$d."', INTERVAL 1 DAY)";
			elseif ( $filter['flags']['Period']['month'] )
				$where[] = " ".$this->_tables['articles'].".`Date` < DATE_ADD('".$d."', INTERVAL 1 MONTH)";
			else
				$where[] = " ".$this->_tables['articles'].".`Date` < DATE_ADD('".$d."', INTERVAL 1 YEAR)";
		}
	...
	public function GetPhotos($filter)
	{
		...
		if ( isset($filter['flags']['PlaceID']) && $filter['flags']['PlaceID'] > 0 )
		...
		if ( $filter['flags']['PlaceID'] != -1 )
			$where[] = ' '.$this->_tables['photos'].'.PlaceID = '.$filter['flags']['PlaceID'];
