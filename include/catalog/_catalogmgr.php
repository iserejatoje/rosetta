...
//                        'price' => $addition->price,
    ...
    public function SlicePrice($products, $from, $to)
    {
        ...
        $sql  = "SELECT * FROM ".$this->_tables['cache_prices'];
        if($from > 0 && $to > 0)
        {
            $sql .= " WHERE MinPrice >= ". intval($from);
            $sql .= " AND MaxPrice <= ".intval($to);
        }
        elseif($from > 0)
        {
            $sql .= " WHERE MinPrice >= ". intval($from);
        ...
        elseif($to > 0)
        {
            $sql .= " WHERE MaxPrice <= ". intval($to);
        }
        $sql .= " AND ProductID in (".implode(', ', $pids).")";
...
//            $params = '';
    ...
    public function GetDiscountCards($filter)
    {
        ...
        if ( isset($filter['field']) ) {
            ...
            foreach($filter['field'] as $k => $v) {
                if ( !in_array($v, array('CardID', 'code', 'created', 'orderid', 'isactive')) )
                    unset($filter['field'][$k], $filter['dir'][$k]);
            }
