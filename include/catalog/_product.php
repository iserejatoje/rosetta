...
class Product
{
    ...
    */
    ...
    public function CachePrice($catalogid = 0)
    {
        ...
        {
            $price_min = $price_max = $this->GetPrice($this->length, $this->count, $catalogid);
        }
        elseif($category->Kind == CatalogMgr::CK_BOUQUET || $category->Kind == CatalogMgr::CK_MONO || $category->Kind == CatalogMgr::CK_FIXED)
        {
            $types = $this->GetTypes();
        ...
        $typeid = $category->id;
        if($this->IsPriceCached($catalogid, $typeid)) {
