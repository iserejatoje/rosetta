...
class SFilter
{
    ...
    function __construct(array $info)
    {
        ...
        $this->cache = ServiceMgr::getInstance()->GetCache();
    }
    ...
    public function Update()
    {
        ...
        {
            return $this->_values['filterid'] = ServiceMgr::getInstance()->AddFilter($this->_values);
        }
        else
        {
            return  ServiceMgr::getInstance()->UpdateFilter($this->_values);
        }
    }
    ...
    public function Remove()
    {
        ...
        return ServiceMgr::getInstance()->RemoveFilter($this->_values['filterid']);
    }
    ...
    public function GetParams($filter)
    {
        ...
        {
            $this->params = array();
            $sql = "SELECT * FROM ".ServiceMgr::getInstance()->_tables['filter_params']." WHERE";
            ...
            if(is_array($filter) && count($filter) > 0)
            {
                ...
                if(isset($filter['field']) && $filter['dir'] && count($filter['field']) == count($filter['dir']))
                {
                    ...
                    {
                        $sqlo[] = ' '.ServiceMgr::getInstance()->_tables['filter_params'].'.`'.$filter['field'][$k].'` '.$filter['dir'][$k];
                    }
    ...
    public function AddParam($name, $options)
    {
        ...
        $sql = "INSERT INTO ".ServiceMgr::getInstance()->_tables['filter_params']." SET";
        $sql .= " FilterID=".$this->_values['filterid'];
        ...
        $id = ServiceMgr::getInstance()->_db->insert_id;
        ...
        $this->cache->Remove('service_filter_params_'.$this->_values['filterid']);
    ...
    public function UpdateParam($id, $name, $options)
    {
        ...
        $sql = "UPDATE ".ServiceMgr::getInstance()->_tables['filter_params']." SET";
        $sql .= " Name='".addslashes($name)."'";
        $sql .= ", Value='".addslashes($options['value'])."'";
        $sql .= ", IsAvailable='".intval($options['isavailable'])."'";
    ...
    public function RemoveParam($id)
    {
        ...
        $sql = "DELETE FROM ".ServiceMgr::getInstance()->_tables['filter_params'];
        $sql .= " WHERE ParamID=".$id;
