<?php
class AppCommentTree {

	public static $db = null;
	public static $slave = true;
	...
	/**
	 * Копирование комментарий и связей для одного UniqueID
	 *
	 * @param srcUniqueId int - UniqueID источника
	 * @param sections array - RefID новый связей
	 * @return bool
	 */
	public static function Copy($srcUniqueId, $sections)
	{
		if (!is_numeric($srcUniqueId) || $srcUniqueId <= 0)
			return false;

		if (!is_array($sections) || count($sections) == 0)
			return false;

		$sql = "SELECT c.* FROM `".self::$tables['comments']."` c";
		$sql.= " INNER JOIN `".self::$tables['ref']."` r ON (c.CommentID=r.CommentID)";
		$sql.= " WHERE r.UniqueID = ".$srcUniqueId;
		...
		{
			$sql = "INSERT INTO `".self::$tables['comments']."` SET";
			$sql.= " `UserID` = '".$row['UserID']."'";
			$sql.= ", `ParentID` = '".$row['ParentID']."'";
			$sql.= ", `Level` = '".$row['Level']."'";
			$sql.= ", `Created` = '".$row['Created']."'";
			$sql.= ", `Text` = '".addslashes($row['Text'])."'";
			$sql.= ", `Name` = '".addslashes($row['Name'])."'";
			$sql.= ", `IsNew` = '".$row['IsNew']."'";
			$sql.= ", `IsVisible` = '".$row['IsVisible']."'";
			$sql.= ", `IsHidden` = '".$row['IsHidden']."'";
			$sql.= ", `IsDel` = '".$row['IsDel']."'";
			$sql.= ", `VotePlus` = '".$row['VotePlus']."'";
			$sql.= ", `VoteMinus` = '".$row['VoteMinus']."'";
			$sql.= ", `VoteCount` = '".$row['VoteCount']."'";
			$sql.= ", `Source` = '".$row['Source']."'";
			$sql.= ", `ChildsCount` = '".$row['ChildsCount']."'";
			self::$db->query($sql);

			$id = self::$db->insert_id;
			$levels[$row['CommentID']] = $id;

			if ($row['ParentID'] != 0 && $row['Level'] > 0 && isset($levels[$row['ParentID']]))
			{
				$sql = "UPDATE `".self::$tables['comments']."` SET";
				$sql.= " ParentID=".$levels[$row['ParentID']];
				$sql.= " WHERE CommentID=".$id;
				self::$db->query($sql);
			}

			$sql = "INSERT INTO `".self::$tables['ref']."` (`CommentID`, `UniqueID`, `opt_Date`) VALUES ";
			foreach($sections as $section)
				$sql.= "(".$id.", ".$section.", '".$row['Created']."'),";
	...
	public static function Remove($ids)
	{
		...
		/*$sql = 'SELECT `ParentID` FROM '.self::$tables['comments'].' WHERE ';
		$sql.= ' CommentID IN('.implode(',', $ids).')';
