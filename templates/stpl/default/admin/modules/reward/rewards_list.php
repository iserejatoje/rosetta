<script>
	...
	$(document).ready(function(){
		...
		$('.table a.delete').click(function(e){
			e.preventDefault();
			if (!confirm("Вы действительно хотите удалить награду?"))
				return false;
			...
			$.ajax({
				...
				success: function(data){
					...
					if (data.is_visible == 1) {
						$('.table #reward_'+data.id+' a.visible-toggler .glyphicon')
...
<form method="POST" name="rewardsform">
<input type="hidden" name="action" value="save_reward_orders" />
...
<table class="--sortable table table-bordered table-hover">
	...
	<tbody>
	<?php $i = 0; ?>
	<?php foreach($vars['rewards'] as $item){ ?>
	<tr class="<?php if ($i %2 == 0) { ?>odd<?php } else { ?>notodd<?php } ?>" id="reward_<?= $item->id?>" data-id="<?=$item->id?>">
		...
		<td class="vert-align">
			<a href="?section_id=<?=$vars['section_id']?>&action=edit_reward&id=<?=$item->id?>">
				<?= $item->title?>
			</a>
		</td>
		<td class="vert-align">
		...
		<td align="center">
			<a href="?section_id=<?=$vars['section_id']?>&action=ajax_reward_toggle_visibility&id=<?=$item->id?>" class="visible-toggler btn btn-default btn-sm">
		...
		<td align="center" class="vert-align">
			<a class="delete" href="?section_id=<?=$vars['section_id']?>&action=delete_reward&id=<?=$item->id?>">
