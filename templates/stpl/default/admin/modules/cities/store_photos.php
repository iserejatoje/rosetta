...
<a class="btn btn-primary btn-sm" href="?section_id=<?=$vars['section_id']?>&action=new_photo&storeid=<?=$vars['storeid']?>" role="button">Добавить фотографию</a>
<br/><br/>
<form method="POST" name="groupsform">
	...
	<table class="sortable table table-bordered table-hover table-striped">
		...
		<tr class="<? if ($i %2 == 0) { ?>odd<? } else { ?>notodd<? } ?>" id="item_<?=$item->ID?>">
			...
			<td align="left">
				<a href="?section_id=<?=$vars['section_id']?>&action=edit_photo&storeid=<?=$vars['storeid']?>&id=<?=$item->ID?>"><?=$item->name?></a>
			</td>
			...
			<td align="center">
				<a href="?section_id=<?=$vars['section_id']?>&action=delete_photo&storeid=<?=$vars['storeid']?>&id=<?=$item->ID?>" class="delete btn btn-default btn-sm">
