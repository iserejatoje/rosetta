<div class="jcmcarousel-skin-title-video">
	<div class="jcmcarousel-container jcmcarousel-container-horizontal">
		<div class="jcmcarousel-prev-container">
			<div class="jcmcarousel-logo"><a href="{$res.Link}" title="Все видео" target="_blank"><img src="/_img/x.gif" width="12" height="90" alt="Все видео" /></a></div>
		...
		{/if}
					...
					<div class="jcmcarousel-item-title">
						{if $l.addmaterial == 3}
							<a href="{$l.url.absolute}#video" title="{$title}" target="_blank" class="redtext">{if strpos($l.url.absolute, 'newyear')}Новогоднее видео{else}Любительское видео{/if}</a>
						{else}
