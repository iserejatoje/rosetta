...
<table width="100%" class="block_right" cellspacing="3" cellpadding="0">
	...
	<tr align="left"><td>
                {$l.Date|date_format:"%e"} {$l.Date|month_to_string:2} {$l.Date|date_format:"%Y"}<br/>
...
<a href="{$l.Link}{$l.NewsID}.html" class="anon"> {/if}<span class="anon_name">
		{if $l.TitleType==2}
			<b>{$l.TitleArr.name}</b>,</span><br/><span class="anon_position">{$l.TitleArr.position}{if $l.TitleArr.text}: <b>{$l.TitleArr.text}</b>{/if}
		{else}
		<b>{$l.Title}</b>
		{/if}</span></a> 
	...
	<tr align="left"><td>
		{if $l.ThumbnailImg.file }
			{if $CURRENT_ENV.site.domain != $ENV.site.domain }<a href="/service/go/?url={"`$l.Link``$l.NewsID`.html"|escape:"url"}">{else}<a href="{$l.Link}{$l.NewsID}.html">{/if}
