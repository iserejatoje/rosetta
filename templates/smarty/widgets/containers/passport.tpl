...
{strip}<div style="position:absolute;right:4px;top:4px;width:69px;height:10px;">
	...
	{if $config.is_configurable}
		<div class="widgetButtonConfigureText widgetButtonConfigure widgetButtonTitle" title="настройка" style="padding-right:2px;"><div style="margin-top: -3px; text-decoration: underline; font-size: 10px; color: #efefef; text-align: left;">настр.</div></div>
	{/if}	
	</div>{/strip}
	<span>{$title}</span>
</div>
