...
<input type=hidden name=action value=subs_dnevniki>
{if count($res.error.err)>0}	
<tr><td align=center bgcolor=#ffffff>{include  file="`$TEMPLATE.errors`" errors_list=$res.error.err}</td></tr>
{/if}
{if $res.list.count>0}
<tr>
<td align="right" class="text11" colspan=2>Всего подписок: {$res.list.count}. Возможно: {$res.list.count_all}.</td>
</tr>
<tr class="block_title2">
	<th width=90%>Название дневника</th>
	<th width=10%>Отписаться</th>
</tr>
{excycle values="#F5F9FA,#FFFFFF"}
{foreach from=$res.list.dnevniki item=l key=k}
<tr bgcolor="{excycle}">
...
<tr>
	<td align=center colspan="2"><input name=btnsub class=button type=submit value="сохранить изменения" style="width:200"></td>
</tr>
{else} 
