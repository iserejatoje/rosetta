...
<form method="POST" enctype="multipart/form-data"{if $res.action} action="{$res.action}"{/if}{if $USER->IsAuth() && ($res.type=='create_theme' || $res.type=='edit_theme')} onsubmit="return checkMsgForm(this);"{/if}>
...
{if $res.type=='create_theme' || ($res.type=='edit_theme' && $res.data.is_theme)}
	<tr>
		...
		<td>{if $UERROR->GetErrorByIndex('title') != ''}
			...
			{/if}
			<input type="text" name="title" style="width:99%" value="{$res.data.title}">
			</td>
	</tr>
	<tr><td>&nbsp;</td><td>&nbsp;</td></tr>
{/if}
	<tr>
        ...
        <td valign="top">
			<style>{literal}
				...
				.ForumEditor .BBbuttons DIV.bold {
					background-position: 0 0;
				}
				.ForumEditor .BBbuttons DIV.italic {
					background-position: -26px 0;
				}
				.ForumEditor .BBbuttons DIV.underline {
					background-position: -52px 0;
				}
				.ForumEditor .BBbuttons DIV.small {
					background-position: -78px 0;
				}
				.ForumEditor .BBbuttons DIV.align_left {
					background-position: -182px 0;
				}
				.ForumEditor .BBbuttons DIV.align_center {
					background-position: -234px 0;
				}
				.ForumEditor .BBbuttons DIV.align_right {
					background-position: -208px 0;
				}
				.ForumEditor .BBbuttons DIV.align_justify {
					background-position: -156px 0;
				}
				.ForumEditor .BBbuttons DIV.email {
					background-position: -104px 0;
				}
				.ForumEditor .BBbuttons DIV.list {
					background-position: -130px 0;
				}
				.ForumEditor .BBbuttons DIV.url {
					background-position: -260px 0;
				}
				.ForumEditor .BBbuttons DIV.media {
					background-position: -286px 0;
				}
				.ForumEditor .BBbuttons DIV.image {
					background-position: -338px 0;
				}
			{/literal}</style>
		
			<div class="BBbuttons">
				<div class="bold" onclick="inserttag('B');" title="жирный"></div>
				<div class="italic" onclick="inserttag('I');" title="курсив"></div>
				<div class="underline" onclick="inserttag('U');" title="подчеркнутый"></div>
				<div class="small" onclick="inserttag('SMALL');" title="маленький"></div>
				<div class="align_left" onclick="inserttag('LEFT');" title="прижат к левому краю"></div>
				<div class="align_center" onclick="inserttag('CENTER');" title="по центру"></div>
				<div class="align_right" onclick="inserttag('RIGHT');" title="прижат к правому краю"></div>
				<div class="align_justify" onclick="inserttag('JUSTIFY');" title="выравнен по обоим краям"></div>
				<div class="email" onclick="inserttag('EMAIL');" title="email"></div>
				<div class="url" onclick="inserttag('URL');" title="ссылка"></div>
				<div class="media" onclick="inserttag('MEDIA');" title="видеоролик"></div>
				<div class="image" onclick="inserttag('IMG');" title="картинка"></div>
...
{assign var=hasimgs value=true}
							...
							<div style="width:200px;"><input type="file" name="img_old[{$img.id}]" style="font-size:12px;"></div>
{else}
							<div style="width:200px;{if $hasimgs==true}height:170px;{/if}">
							</div>
							<div style="width:200px;"><input type="file" name="img[]" style="font-size:12px;"></div>
{/if}
						</div>
{/foreach}
