	...
	<tbody id="pl{$res.placeinfo.fix}">
		...
		<tr>
			...
			<td class="bg_color4">
				...
				{if $res.placeinfo.placeid}
					<input type="hidden" value="{$res.placeinfo.citycode}" id="city{$res.placeinfo.fix}" name="place[{$res.placeinfo.type}][city][]" />
				{/if}
		...
		<tr>
			...
			<td class="bg_color4">
				<input type="hidden" name="place[{$res.placeinfo.type}][uniqueid][]" value="{$res.placeinfo.uniqueid}" />
		...
		<tr>
			...
			<td class="bg_color4">
				<select name="place[{$res.placeinfo.type}][y_start][]" style="width:18%;">
					<option value="0">-год-</option>
					{foreach from=$res.years_arr item=w
						}<option value="{$w}"{if $w==$res.placeinfo.y_start} selected="selected"{/if}>{$w}</option>{
					/foreach}
				</select>
				<select name="place[{$res.placeinfo.type}][m_start][]" style="width:24%;">
				...
				<select name="place[{$res.placeinfo.type}][m_end][]" style="width:24%;"{*if $res.placeinfo.m_end==0} disabled="true"{/if*}>
					<option value="0">-месяц-</option>
					{foreach from=$res.months_arr key=m item=w
