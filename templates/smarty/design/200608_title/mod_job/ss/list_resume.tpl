...
{if sizeof($res.res)}
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="table2">
		<tr>
			...
			<th width="50%" style="border-right: solid 2px #fff;">Имя{*Ф.И.О.*}</th>
		...
		{foreach from=$res.res item=l name=listrows}
			{capture name="link"}{if $ENV.regid == 74 && $l.rid == 5}http://cheldoctor.ru/job/
			{elseif $ENV.regid == 74 && in_array($l.rid,array(1,11,22))}http://chel.ru/job/
				{elseif in_array($l.rid,array(13,21,32))}
					{if $ENV.regid == 74}http://chelfin.ru/job/
					{elseif $ENV.regid == 2}http://102banka.ru/job/
					{elseif $ENV.regid == 16}http://116dengi.ru/job/
					{elseif $ENV.regid == 61}http://161bank.ru/job/
					{elseif $ENV.regid == 34}http://34banka.ru/job/
					{elseif $ENV.regid == 59}http://dengi59.ru/job/
					{elseif $ENV.regid == 63}http://dengi63.ru/job/
					{elseif $ENV.regid == 72}http://72dengi.ru/job/
					{/if}
			{else}/{$ENV.section}/{/if}{/capture}
			{capture name="target"}{if in_array($ENV.regid,array(74,2,16,61,34,59,63,72)) && in_array($l.rid,array(1,5,11,13,21,22,32))} target="_blank"{/if}{/capture}
			<tr valign="top" class="{excycle}">
			      ...
			      <td style="border-right: solid 2px #fff;">{if $l.imp}<font color=red>{/if}{$l.fio}{if $l.imp}</font>{/if}</td>
			      <td style="border-right: solid 2px #fff;">
					<a class="list" href="{$smarty.capture.link}resume/{$l.resid}.html{if $l.archive == 1}?archive{if $l.rid}&rid={$l.rid}{/if}{elseif $l.rid}?rid={$l.rid}{/if}"{$smarty.capture.target}>{if $l.imp}<font color="red">{$l.dol}</font>{else}{$l.dol}{/if}</a><br/>
			...
			<tr class="{excycle}" valign="top"><td colspan="4" align="center" style="padding:0px;"><div id="resume{$l.resid}loader" style="display:none"></div><div id="resume{$l.resid}content" style="display:none"></div></td></tr>
...
<br /><br />
<img height="79" border="0" width="490" src="/_img/modules/job/money/top_row.jpg" alt=""/>
