		...
		<td align="center">
			...
			{foreach from=$res.data item=l name=lst}
				<tr class="{excycle}" valign="top" {if $l.moderate != 0 || $USER->ID == 1}style="background-color:#FFAEAA"{/if}>
					...
					<td{if ($l.hide || $l.is_new == 2) && $l.moderate == 0} style="color:#999999"{/if}>{$l.firm}<br/>{$l.name}</td>
					<td><a href="/{$ENV.section}/my/vacancy/edit/{$l.vid}.php"{if ($l.hide || $l.is_new == 2) && $l.moderate == 0} style="color:#999999"{/if}>{$l.dol}</a></td>
