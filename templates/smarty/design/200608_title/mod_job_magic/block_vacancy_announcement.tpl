{if $CURRENT_ENV.regid == 74}
	...
	<b><a href="http://www.ukon.su/" style="color:red" target="_blank">Консультация юриста по трудовому праву</a></b><br/><br/></div>

	{if sizeof($res)}
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="block_left">
	<tr>
		<th><span>Вакансии агентств</span></th>
	</tr>
	</table>
			...
			<table cellpadding="0" cellspacing="3" border="0" width="100%" class="table"> 
				...
				{foreach from=$res item=l}
					...
					<tr valign="bottom" class="{excycle}">
						<td><a href="{if $l.link}http://{$l.link}{else}/{$ENV.section}/vacancy/firm/{$l.fid}.php{/if}" alt="{$l.fname|escape}{if !$l.link} ({$l.count|number_format:"0":",":" "}){/if}" title="{$l.fname|escape}{if !$l.link} ({$l.count|number_format:"0":",":" "}){/if}" {if $l.link}target="_blank"{/if}>{$l.fname}</a> <span class="class="text11"">{if !$l.link}({$l.count|number_format:"0":",":" "}){/if}</span></td>
					</tr>
					{/if}
				{/foreach}
				</table>
...
{else}
	...
	<tr>
		<td class="block_title"><span>Вакансии агентств</span></td>
	</tr> 
	</table>
