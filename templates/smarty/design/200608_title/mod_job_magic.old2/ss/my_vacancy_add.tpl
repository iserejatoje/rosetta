					...
					<td class="bg_color4">
						<select id="city_id" name="city_id" size="1" style="width:200px" onchange="mod_job.change_city(this, 'region', 'city');">
							{foreach from=$page.city_arr item=v key=k}
								{if ($ENV.site.domain=="mgorsk.ru" && $k==2376) || ($ENV.site.domain=="74.ru" && $k!=2376) || ($ENV.site.domain=="tolyatty.ru" && $k==1748) || ($ENV.site.domain=="63.ru" && $k!=1748) || ($ENV.site.domain=="ufa1.ru" && $k!=399) || ($ENV.site.domain=="sterlitamak1.ru" && $k==399) || !in_array($ENV.site.domain,array('mgorsk.ru','74.ru','tolyatty.ru','63.ru','ufa1.ru','sterlitamak1.ru'))}
...
трудом;</span>
				...
				<tr>
					...
					{if is_array($page.vacancy.email)}
						...
						<span>
							<input name="email[]" value="{$mail|trim}" style="width:95%">{if $_k > 0}<a onclick="this.parentNode.parentNode.removeChild(this.parentNode)" href="javascript:void(0)" title="Удалить E-Mail"><img src="/img/20061008_2pages/bullet_delete.gif" hspace="4" border="0" alt="Удалить E-Mail"/></a>{/if}</br>
						</span>
						{/foreach}
					{else}
						<span>
