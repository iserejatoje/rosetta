...
{else}
	...
	<tr>
		...
		<td>{$res.regions[$res.region].name}</td>
	</tr>
	{/if}
	{if $res.grafik}
	<tr>
	...
	<tr>
		...
		<td>{$res.uslov|nl2br}</td>
	</tr>
	{/if}
	{if $res.treb}
	<tr>
		<td align='right' width="130"><b>Требования:</b></td>
		<td>{$res.treb|nl2br}</td>
	</tr>
	{/if}
	<tr>
		<td colspan="2" align="right"><a href="{$smarty.capture.link}vacancy/{$res.vid}.html{if $res.rid}?rid={$res.rid}{/if}" class="tip"{$smarty.capture.target}>подробнее...</a></td>
	</tr>
	</table>
{/if}
