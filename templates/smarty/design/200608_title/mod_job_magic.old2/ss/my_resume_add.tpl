<script type="text/javascript" language="javascript" src="/_scripts/modules/job/checkres2.js"></script>
		...
		<td width="100%">
			<table cellpadding="0" cellspacing="2" border="0" width="100%" class="table2">
				...
				<tr>
					...
					<td class="bg_color4">{if $page.vacancy.city_id}{$page.city_arr[$page.vacancy.city_id].name}{else}{$page.vacancy.city}{/if}</td>
				</tr>
				{/if}
			...
			{else}
				<tr>
					...
					<td class="bg_color4">
						...
						<input type="hidden" name="city" value="{$page.resume.city|escape:"html"}"/>{if isset($page.city_arr[$page.resume.city_id])}{$page.city_arr[$page.resume.city_id].name}{elseif !$page.resume.city_id}{$page.resume.city}{else}{$page.city_arr[$CONFIG.default_city].name}{/if}</td>
				</tr>
			{/if}
				<tr>
...
<span style="color:#808080">должен состоять из символов , 0-9, -, ( ) и пробел</span>
				...
				{*<tr>
					...
					<td class="bg_color4" width="100%" align="right">
						<a href="javascript:void(0)" onclick="addEmailField()" title="Добавить E-Mail"><img src="/img/20061008_2pages/bullet_add.gif" border="0" alt="Добавить E-Mail"/></a>
					</td>
				...
				<tr>
					...
					{foreach from=$page.resume.email item=mail key=k}
						<span>
							<input name="email[]" value="{$mail|trim}" style="width:100%">{if $k>0}<a onclick="this.parentNode.parentNode.removeChild(this.parentNode)" href="javascript:void(0)" title="Удалить E-Mail"><img src="/img/20061008_2pages/bullet_delete.gif" border="0" alt="Удалить E-Mail"/></a>{/if}</br>
						</span>
					{/foreach}
					{else}
						<span>
