...
function CountForm() {
	...
	if (!Number(cost.value) && Number(counts.value)==1 && !err) {
		alert("Укажите \"Стоимость квартиры\"!");
		err++;
		cost.focus();
	}
...
function ShowMonthCost() {
	...
	var temp = "<table border=0 cellspacing=1 cellpadding=3 bgcolor=\"#C6E4E2\">";
	for (var i = Number(b_date.value); i <= Number(e_date.value); i++) {
		...
		temp += "<td  width=120>"+FormatNum(flat_cost)+"</td>";
		temp += '<td  width=80>'+FormatNum(flat_init)+'</td>';
		temp += '<td  width=60>'+FormatNum(credit)+'</td>';
		temp += "<td  width=65>"+FormatNum(overpay)+"</td>";
		temp += "<td width=60>"+overpay_percent+"</td>";
   		temp += "</tr>";
	}  
	temp +="</table>";
	return temp;
...
<br/>
			...
			<tr><td bgcolor="#C6E4E2" align=center class="text11" height=20><b>Расчет суммы ипотечного кредита</b></td></tr>
			<tr>
				<td nowrap>
				...
				<tr>
					...
					<td colspan="3"><select id="counts" name="counts" class=text11 style="width:270px;" onchange="HideField(this.selectedIndex)">{foreach from=$res.ipocount item=l key=k}<option value="{$k}">{$l}</option>{/foreach}</select></td>
				</tr>
				<tr>
					<td align=right class=text11>Cрок кредита от</td>
					<td nowrap><input type=text name="b_date" id="b_date" maxlength=2 size=6 class=text11 value=""  style="width:20px;" onkeypress="return CheckInt(event, this);"> до <input type=text id="e_date" name="e_date" maxlength=2 size=6 class=text11 value=""  style="width:20px;" onkeypress="return CheckInt(event, this);"> лет</td>
				...
				<tr>
					<td align=right class=text11 nowrap>Стоимость квартиры:</td>
		...
		<div id="counts0" style="display: none;"><br/>
			...
			<th width=120 class="text11">Стоимость квартиры,<br/><span id="val5">руб.</span></th>
			<th width=80 class="text11">Первонач. взнос,<br/><span id="val6">руб.</span></th>
			<th width=60 class="text11">Кредит,<br/><span id="val7">руб.</span></th>
			<th width=65 class="text11">Переплата в <span id="val8">руб.</span></th>
			<th width=60 class="text11">Переплата в %</th>
   			</tr>			
			</table>
		</div>
