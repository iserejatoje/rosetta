...
{if $res.in_state == 1}
	...
	<tr>
		...
		<td class="bg_color4">{include file="`$CONFIG.templates.ssections.branches`"}</td>
	</tr>
	<tr>
...
{else}
	...
	<tr>
		...
		<td class="bg_color4">{$res.addr}{if $res.map_link} <a href="{$res.map_link}" target="_blank">Найти на карте</a>{/if}</td>
	</tr>
	{/if}
	{if count($res.files) > 0}
	<tr>
		<td class="bg_color2" align="right" width="130">Прикреплённые файлы:</td>
		<td class="bg_color4">			
			{foreach from=$res.files item=file key=k}
	...
	<tr>
		<td colspan="2" align="left" class="text11">
			Это объявление некорректно? <a href="#" onclick="mod_job_incorrect_obj.show({$res.vid}, 'vacancy'); return false;">Сообщите нам об этом</a>.
	...
	<tr>
		<td colspan=2 style="background-color:#EDF6F8; color: red; text-align: center;">Внимание! Не станьте жертвой мошенников, которые снимают деньги с сотовых телефонов. Не вводите номера своих телефонов для получения кода доступа на сомнительных сайтах, не отправляйте СМС для просмотра вакансии. Не ведитесь на действия мошенников при поиске работы, будьте бдительны.<br />Сайт не несет ответственность за содержание объявлений!</td>
	</tr>	
