		...
		{foreach from=$res.res item=l name=listrows}
			...
			<tr valign="top" class="{excycle}">
			      ...
			      <td style="border-right: solid 2px #fff;">
					<a class="list" 
						href="{$smarty.capture.link}resume/{$l.resid}.html{if $l.archive == 1}?archive{if $l.rid}&rid={$l.rid}{/if}{elseif $l.rid}?rid={$l.rid}{/if}"
						{$smarty.capture.target}><b>
							{if $l.imp}<font color="red">{/if}
								{if $l.dol != ""}
									{$l.dol}
								{else}
									{php} $this->_tpl_vars['aid'] =  $this->_tpl_vars['l']['resid'] {/php}
			...
			<tr>
				<td colspan="4" align="center">
				{include file="`$TEMPLATE.lastbanner2`"}
				</td>
			</tr>
			{/if}
