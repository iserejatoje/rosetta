...
{* Top title *}
	...
	{$ENV._arrays.rubrics[$ENV._params.rubric]}&nbsp;{$ENV._arrays.status_view[$ENV._params.status].z}

	{if $page.advs_count.rent[$ENV._params.rubric][$ENV._params.status][0] !=""}

		&nbsp;<font color="red">{$page.advs_count.rent[$ENV._params.rubric][$ENV._params.status][0]}
		{if $page.hours_count.rent[$ENV._params.rubric][$ENV._params.status]}
			(+{$page.hours_count.rent[$ENV._params.rubric][$ENV._params.status]})
		{/if}
		</font>
	{/if}</b>
		</td>
...
{if $USER->IsAuth()}&nbsp;|&nbsp;
<a href="/{$ENV.cp_section}/" class="data">редактировать объявление</a>
{/if}
&nbsp;|&nbsp;
...
{foreach from=$ENV._arrays.rubrics item=_v key=_k}
	{if $_k == $ENV._params.rubric}
		<div align="right" class="text11">&nbsp;{$_v} {$page.advs_count.rent[$_k][0][0]}
        	{if $page.hours_count.rent[$_k][0]}&nbsp;(<b>+{$page.hours_count.rent[$_k][0]}</b>){/if}
        </div>
	{else}
		<div align="right">
			<a href="/{$ENV.section}/list.html?rubric={$_k}&status={$ENV._params.status}&view={$ENV._params.view}" class="t11_grey">{$_v} {$page.advs_count.rent[$_k][0][0]}
                {if $page.hours_count.rent[$_k][0]}&nbsp;(<b>+{$page.hours_count.rent[$_k][0]}</b>){/if}
            </a>
        </div>
	{/if}
...
{if !$ENV._params.object || $ENV._arrays.objects[$ENV._params.object].n > 0}
	{foreach from=$ENV._arrays.status_view item=_v key=_k}
	  {if $_k == $ENV._params.status}
			&nbsp;{$_v.m}
	                {if $page.advs_count.rent[$ENV._params.rubric][$_k][0] && $_k}
	                             &nbsp;{$page.advs_count.rent[$ENV._params.rubric][$_k][0]}
	                {/if}
	{else}
	        		<a href="/{$ENV.section}/list.html?rubric={$ENV._params.rubric}&status={$_k}&view={$ENV._params.view}" class="button">{$_v.m}
	               	{if $page.advs_count.rent[$ENV._params.rubric][$_k][0] && $_k}
	                             &nbsp;{$page.advs_count.rent[$ENV._params.rubric][$_k][0]}
	                {/if}
	                </a>
