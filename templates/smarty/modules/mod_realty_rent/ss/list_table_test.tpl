...
<table width="100%" cellspacing="2" border="0" class="table2">
	<tr>
		...
		<th width="40px">Телефон</th>
		<th width="40px">Мебель</th>
	</tr>
...
{foreach from=$page.list item=l key=_k}
	...
	<tr class="{if $_k%2}bg_color4{/if}" align="center">
		...
		{if $l.address}
			...
			{/if}
		{/if}&nbsp;{if $l.agent}&nbsp;<img src="/_img/modules/realty/icon_{if $l.agent==1}s{else}a{/if}.png" alt="От {$ENV._arrays.agent[$l.agent].b}" title="От {$ENV._arrays.agent[$l.agent].b}">{/if}{if !empty($l.img1) || !empty($l.img2) || !empty($l.img3)}&nbsp;<a href="/{$ENV.section}/details.html?id={$l.id}#photo"><img src="/_img/design/200608_title/common/photo_blue.gif" width="14" height="10" alt="Есть фото" title="Есть фото" border="0"></a>{/if}
		{if $l.area_build}
			<br />Площадь помещения:&nbsp;
		...
		{/if}
			...
			{$l.contacts|strip_tags|truncate:60}
		...
		<td{if $l.imp>0} style="color:red"{/if}>{if $ENV._arrays.regions[$l.region].s}{$ENV._arrays.regions[$l.region].s}{else}-{/if}</td>
		<td{if $l.imp>0} style="color:red"{/if}>{if $l.floor}{$l.floor}{else}-{/if}/{if $l.floors}{$l.floors}{else}-{/if}</td>
		<td{if $l.imp>0} style="color:red"{/if}>{$ENV._arrays.phone[$l.phone]}</td>
		<td{if $l.imp>0} style="color:red"{/if}>{$ENV._arrays.furnit[$l.furnit].s}</td>
	</tr>
