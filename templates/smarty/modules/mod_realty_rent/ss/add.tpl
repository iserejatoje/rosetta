...
{foreach from=$ENV._arrays.regions item=v key=k}
	<option value="{$k}" {if $k ==$ENV._POST.region}{assign var=any value=false}selected="selected"{/if}>{$v.b}</option>
{/foreach}
<option value="0" {if $any}selected="selected"{/if}>-- укажите --</option>
						</select>
      				</td>
...
{/foreach}
				...
				<tr class="bg_color4">
      				...
      				<td><select name=decoration style=width:300px>
{assign var=any value=true}
...
{/foreach}
	<option value="0" {if $any}selected="selected"{/if}>-- укажите --</option>
				...
				<tr class="bg_color4">
      				...
      				<td><input type="text" name="address" style="width:295px" value="{$ENV._POST.address|strip_tags|htmlspecialchars}" maxlength="255"></td>
    			</tr>
    			<tr class="bg_color4">
...
{/foreach}
	...
	<tr class="bg_color4">
      				...
      				<td><select name=phone style=width:300px>
...
<tr class="bg_color4">
      				<td align=right><b>Наличие мебели <font style="color: red; font-weight: normal;">*</font>:</b></td>
      				<td><select name=furnit style=width:300px>
{assign var=any value=true}
{foreach from=$ENV._arrays.furnit item=v key=k}
	<option value="{$k}" {if $k == $ENV._POST.furnit}selected="selected"{assign var=any value=false}{/if}>{$v.b}</option>
{/foreach}
...
<tr>
      ...
      <td><textarea name=description style=width:295px;height:50px>{$ENV._POST.description|strip_tags}</textarea></td>
    </tr>
    <tr>
...
{/foreach}
    ...
    <tr class="bg_color4">
      ...
      <td><textarea name=contacts style=width:295px;height:50px>{if $ENV._POST.contacts}{$ENV._POST.contacts|strip_tags}{else}{$page.user.contacts}{/if}</textarea></td>
    </tr>
    <tr>
...
<tr>
      ...
      <td><input type=text name=email style=width:295px value="{$ENV._POST.email|strip_tags}"></td>
</tr>
<tr>
