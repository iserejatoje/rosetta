...
<tr valign="middle"> 
	<td style="padding-left:10px;padding-right:10px;" class="bg_color2">
		...
		<tr>
			<td align="left">&nbsp;</td>
			<td align="center" style="padding-left:10px;" class="txt_color4"><font class="text11">{if $CURRENT_ENV.regid==74}<a href="/{$ENV.section}/top.php">Top 100 запросов</a>{else}&nbsp;{/if}</font></td>
		</tr>
		<tr>
			<td align="left" colspan="2"><img src="/_img/x.gif" width="1" height="5" border="0" /></td>
		</tr>
		<tr> 
			<td width="100%"><input type="text" name="query" id="query" style="width:100%;" value="{$res.query}" /></td>
			<td align="center" style="padding-left:10px;"><input type="submit" style="width:130px;" value="Найти" /></td>
		</tr>
		<tr>
			<td align="left">
			<input onclick="_ChangeWhere(this.form);" type="radio" id="where_3" name="where" value="3"{if !in_array($res.where, array(1, 2)) || $res.where == 3} checked="checked"{/if} /><label for="where_3">на {$CURRENT_ENV.site.domain}</label> (<a href="/{$ENV.section}/{$CONFIG.files.get.advanced.string}?{$res.advanced_query_string}">расширенный</a>)
			&nbsp;&nbsp;
