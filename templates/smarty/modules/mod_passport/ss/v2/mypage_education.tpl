...
{php}
	$this->_tpl_vars['res'] = array(
		...
		'city_arr'		=> (( isset($this->_tpl_vars['page']['form']['city_arr'][$this->_tpl_vars['l']['countrycode']]) ) ? $this->_tpl_vars['page']['form']['city_arr'][$this->_tpl_vars['l']['countrycode']] : array()),
...
<tr><td align="right">
		...
		<a id="append_education_{php}echo PlaceSimpleMgr::PT_HIGHER_EDUCATION;{/php}" href="javascript:void(0)" 
			onclick="mod_passport.getPlaceForm({php}echo PlaceSimpleMgr::PT_HIGHER_EDUCATION;{/php}, this)" class="text11">Добавить еще один ВУЗ</a>
...
<tr><td align="right">
		...
		<a id="append_education_{php}echo PlaceSimpleMgr::PT_SECONDARY_EDUCATION;{/php}" href="javascript:void(0)" 
			onclick="mod_passport.getPlaceForm({php}echo PlaceSimpleMgr::PT_SECONDARY_EDUCATION;{/php}, this)" class="text11">Добавить еще одну школу</a>
...
onclick="mod_passport.getPlaceForm({$top.TypeID}, $('#append_education_{$top.TypeID}'), {$top.PlaceID});">{$top.Name}</a>{if $top.UsersCount > 0} <sup class="tip"> <a href="/{$CURRENT_ENV.section}/{$CONFIG.files.get.users_place.string}?id={$top.TypeID}&val={$top.PlaceID}" title="Список пользователей" class="text11" style="color: red">{$top.UsersCount|number_format:0:'':' '}</a> </sup>{/if
			}{if !$smarty.foreach.top.last},{/if}
		{/foreach}
	</div>
{/if}
