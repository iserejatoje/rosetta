...
<tr class="{if $l.fixed!=0}frow_fixed{elseif $l.closed!=0}frow_closed{else}{excycle}{/if}">
	...
	<td>
		...
		<a href="{$CONFIG.files.get.theme.string}?id={$l.id}"><b>{$l.name}</b></a><br>{*{$l.descr}*}
		{if !empty($l.pages)}<span class="small">Страницы: {foreach from=$l.pages item=p}{if $p=='...'}, ...{else}{if $p!=1},{/if} <a href="{$CONFIG.files.get.theme.string}?id={$l.id}&p={$p}">{$p}</a>{/if}{/foreach}</span>{/if}</td>
