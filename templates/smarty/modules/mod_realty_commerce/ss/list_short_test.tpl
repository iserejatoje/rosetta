...
<table width="100%" cellspacing="1" border="0" class="table2">
	<tr bgcolor="#E9EFEF" align="center">
		...
		<th width="40px" align="center"><b>Площадь<br/>помещения<br/>(кв.м.)</b></th>
	</tr>
...
{foreach from=$page.list item=l key=_k}
	...
	<tr class="{if $_k%2}bg_color4{/if}" align="center">
		<td class="text11">
			<span class="s3" color="red">{$l.date_start|simply_date:"%f":"%d-%m"}</span><br />{$l.date_start|date_format:"%H:%M"}{* "H:i"|date:$l.date_start *}
		</td>
		<td align="left">
		{if $l.address}
		...
		<td{if $l.imp>0} style="color:red"{/if}>{if intval($l.area_build) < floatval($l.area_build)}
