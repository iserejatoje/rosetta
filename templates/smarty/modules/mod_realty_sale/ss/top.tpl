...
{* Top title *}
	...
	{if $page.advs_count.sale[$ENV._params.rubric][$ENV._params.status][0] !=""}
		...
		&nbsp;<font color="red">{$page.advs_count.sale[$ENV._params.rubric][$ENV._params.status][0]} 
...
{if is_array($ENV._arrays.sale_rub)}
		<td nowrap="nowrap">

{foreach from=$ENV._arrays.sale_rub item=_v key=_k}
	{if $_k == $ENV._params.rubric}
		<div align="right" class="text11">&nbsp;{$_v} {$page.advs_count.sale[$_k][0][0]}
        	{if $page.hours_count.sale[$_k][0]}&nbsp;(<b>+{$page.hours_count.sale[$_k][0]}</b>){/if}
        </div>
	{else}
		<div align="right">
			<a href="/{$ENV.section}/list.html?rubric={$_k}&status={$ENV._params.status}&view={$ENV._params.view}" class="text11">{$_v} {$page.advs_count.sale[$_k][0][0]}
                {if $page.hours_count.sale[$_k][0]}&nbsp;(<b>+{$page.hours_count.sale[$_k][0]}</b>){/if}
            </a>
        </div>
	{/if}
...
{* это условие никогда не выполниться и вообще непонятен его смысл...
Прежде чем раскомментировать - обратитесь к Николаю Диденко
