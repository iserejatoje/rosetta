...
{/if}
	...
	<tr>
		...
		{if !$BLOCK.favorites}
		<th width="70">Продлить<br/><input type=checkbox value="{$row.id}" title="Продлить все объявления на неделю" onclick="mod_realty.selAll('prolong',this)"></th>
		{/if}
		<th width="70">Удалить<br/><input type=checkbox value="{$row.id}" title="Удалить все объявления" onclick="mod_realty.selAll('del',this)"></th>
		{/if}
	</tr>
