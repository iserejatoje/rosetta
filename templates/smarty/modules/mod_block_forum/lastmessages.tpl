...
{foreach from=$res.messages item=l}
		{*<div style="padding:2px 0px 2px 0px;">
			...
			&nbsp;&nbsp;<a href="{$res.url}theme.html?id={$res.theme}&act=last">{$l.message|strip_tags|truncate:50}</a>
		</div>*}
		<div class="block_title2">
			<a name="{$l.id}"></a><span class="title_normal">{$l.count}.</span><span class="comment_name"><b>{$l.login|truncate:20}</b></span>&nbsp;&nbsp;<span class="comment_date">{$l.created|simply_date}</span>
		</div>
		<div style="padding-left:20px;">{$l.message|strip_tags}</div>
		<div>&nbsp;</div>
{/foreach}
{if $res.showlinkall && is_array($res.pageslink.btn) && sizeof($res.pageslink.btn) != 0}
		<div style="padding:2px 0px 2px 0px;" align="right">
			<noindex><a href="{$res.url}theme.html?id={$res.theme}">Все мнения</a></noindex>
		</div>
{/if}
{if $res.onlytext==false}
</td></tr>
