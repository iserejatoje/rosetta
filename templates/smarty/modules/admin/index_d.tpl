...
function ShowPopupMenu(obj, type, id)
{
	...
	if({$l.id}==type)
		text+="{$l.text}";
	{/foreach}
	{literal}text+= Separator()+"<tr><td onmouseover=\"SelectElement(this)\" class=\"esimple\">Свойства</td></tr>";
	text+= "</table>";
	o_menu = document.getElementById("menu");
	o_menu.innerHTML = text;
	o_menu.style.left = event.clientX - 1;
	o_menu.style.top = event.clientY - 1;
...
<body onload="Init()" onmousemove="MouseMove()" style="margin:0px">zdfhzdfhzdfh
<div class="popupmenu" id="menu" name="menu" onmouseout="HideMenu()"></div>
<table width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td width="180" valign="top"><div style="width:180px;height:100%;overflow-y:auto;overflow-x:scroll">{$menu}</div></td>
		<td width="1" valign="top" bgcolor="#eeeeee" style="overflow:hidden">&nbsp;</td>
		<td height="100%"><iframe src="main_d.php{if $SECTION_ID!=null}?section_id={$SECTION_ID}{/if}" width="100%" height="100%" frameborder="no" name="mainView" id="mainView"></iframe></td>
	</tr>
</table>
