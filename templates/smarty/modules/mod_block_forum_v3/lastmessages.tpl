...
{foreach from=$res.messages item=l}
		...
		<tr valign="top">
			...
			<td><a href="{$l.InfoUrl}" target="_balnk"><img style="margin-right: 5px;" src="{$l.avatar.Url}" width="{$l.avatar.Width}" height="{$l.avatar.Height}" alt="" border="0" /></a></td>
			{/if}
		    <td width="100%">
		<div class="block_title2" style="position:relative;">
			<a name="{$l.id}"></a><span class="title_normal">{$l.count}.</span> <span class="comment_name"><b>{if $l.user > 0}<a href="{$l.InfoUrl}" target="_blank">{$l.login|truncate:20}</a>{else}{$l.login|truncate:20}{/if}</b></span>&nbsp;&nbsp;<span class="comment_date">{$l.created|simply_date}</span>
