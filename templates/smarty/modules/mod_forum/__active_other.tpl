{foreach from=$sections item=l key=section}
 ...
 <div class="fheader_stitle"><a href="{$urls[$section]}" class="fheader_stitle">{$section}</a></div>
{foreach from=$l item=l2}
	{if $l2.level==0}<nobr><a href="{$urls[$section]}view.html?id={$l2.id}">{$l2.data.title}</a>&nbsp;</nobr>{/if}
{/foreach}
&nbsp;<a href="{$urls[$section]}active.html"><font color="red">Активные&nbsp;дискуссии</a></nobr>
</div><br>
...
{foreach from=$themes[$section] item=l}
<tr class="{excycle}">
	...
	<td>
		...
		{if $l2.data.type=='section'}<a href="{$urls[$section]}view.html?id={$l2.id}">{/if}{$l2.data.title}{if $l2.data.type=='section'}</a>{/if} &gt; 
		{/foreach}</span><br>
		...
		<a href="{$urls[$section]}theme.html?id={$l.id}&act=last"><b>{$l.name}</b></a>
	</td>
	<td align="center">{if $l.user}<a href="{$urls[$section]}profile.html?user={$l.user}" target="_blank">{/if}{$l.login}{if $l.user}</a>{/if}</td>
	<td align="center">{$l.messages|number_format:0:" ":" "}</td>
	<td align="center">{$l.views|number_format:0:" ":" "}</td>
	<td>{if $l.last_user}<a href="{$urls[$section]}profile.html?user={$l.last_user}" target="_blank">{/if}<b>{$l.last_login}</b>{if $l.last_user}</a>{/if}<br><span class="small">{$l.last_date|simply_date}</span></td>
</tr>
{/foreach}
</table><br>
{/foreach}
