...
class AdminModule
{
    ...
    function GetPage()
    {
        ...
        switch($this->_page)
        {
            ...
            case 'addition_price':
                $this->_title = "Перенос цен";
                $html = $this->_GetAdditionPrice();
                break;
    ...
    function GetTabs()
    {
        ...
        if(App::$User->IsInRole('u_order_nsk') || App::$User->IsInRole('u_order_nsk')) {
            $filter['flags']['CatalogID'] = $this->_id;
        }
        ...
        if(App::$User->IsInRole('u_order_manager') || App::$User->IsInRole('u_order_nsk') || App::$User->IsInRole('u_order_kemerovo')) {
        ...
        if(App::$User->IsInRole('e_adm_execute_section') && App::$User->IsInRole('e_adm_execute_users')) {
            ...
            $tabs['config_settings'] = ['name' => 'action', 'value' => 'config_settings', 'text' => 'Настройки раздела'];
            $tabs['copy'] = ['name' => 'action', 'value' => 'copy', 'text' => 'Копировать каталог'];
    ...
    private function _PostAction()
    {
        ...
        switch($_POST['action'])
        {
            ...
            case 'copy_addition':
                if ($this->_PostCopyAdditions())
                    Response::Redirect('?'.$DCONFIG['SECTION_ID_URL'].'&action=copy');
                break;
            ...
            case 'copy_types':
                if ($this->_PostCatalogCopyTypes())
                    Response::Redirect('?'.$DCONFIG['SECTION_ID_URL'].'&action=catalog');
                break;

            case 'recache_prices':
                if ($this->_PostCatalogRecachePrices())
                    Response::Redirect('?'.$DCONFIG['SECTION_ID_URL'].'&action=copy');
                break;
...
echo "OK"; exit;
    ...
    private function _PostCopyAdditions()
    {
        global $CONFIG, $OBJECTS;
        ...
        $this->catalogMgr->CopyAdditionsRefs($catalogID, $this->_id, $clear);
    ...
    private function _PostCatalogCopyTypes()
    {
        global $CONFIG, $OBJECTS;
        ...
        $this->catalogMgr->CopyCatalogTypes($catalogID, $this->_id, $clear);

        return true;
    }

    public function _PostCatalogRecachePrices()
    {
        global $CONFIG, $OBJECTS;
        ...
        $this->catalogMgr->RecachePrices($catalogId, $this->_id, $clear);
exit;
        return true;
    }
    ...
    private function _GetAdditionPrice()
    {
     exit;
        ...
        foreach($additions as $addition) {
            ...
            $ref = [
                'SectionID' => $areaRefs['SectionID'],
                'IsVisible' =>  $areaRefs['IsVisible'],
                'InCard' =>  $areaRefs['InCard'],
                'Price' =>  $addition->Price,
            ];

            $addition->UpdateAreaRef($ref);
        }
echo 'ok';
        exit;
    }
    ...
    private function _GetAdditionEdit()
    {
        ...
        if (App::$Request->requestMethod === Request::M_POST) {
            ...
            $form['Price'] = $additionAreaRefs['Price'];
        }
...
<<<<<<< HEAD
            ...
            $info['CorrectionCall'] = 2;
            $info['UserID'] = App::$User->ID;
            $info['PaymentType'] = CatalogMgr::PT_ONLINE;

            if($info['CardID']) {
                $info['CardName'] = $this->catalogMgr->GetCard($info['CardID'])->Name;
            }
...
=======
            ...
            $composition = $this->getOrderComposition(App::$Request->Post->AsArray(null, Request::VALUE | Request::OUT_HTML));
...
>>>>>>> 8d20472f0beb6ee6dba440c5e8e313ff3857583f
