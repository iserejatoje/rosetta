...
if(1)
{
    ...
    define('ERR_A_CUSTOM_ORDER_DELIVERY_DATE_EMPTY', ERR_A_CATALOG_MASK | $error_code++);
    UserError::$Errors[ERR_A_CUSTOM_ORDER_DELIVERY_DATE_EMPTY] = 'Не указана дата';
    ...
    define('ERR_A_PRODUCT_COMPOSITION_IS_EMPTY', ERR_A_CATALOG_MASK | $error_code++);
    UserError::$Errors[ERR_A_PRODUCT_COMPOSITION_IS_EMPTY] = 'Состав заказ не должен быть пустым';
}
...
class AdminModule
{
    ...
    function GetPage()
    {
        ...
        {
            case 'mailer':
                $this->_title = "Тест почты";
                $html = $this->_GetMailing();
                break;
            case 'discountcards':
            ...
            case 'ajax_calc_order_price':
                $this->_GetAjaxCalcOrderPrice();
                break;
            ...
            case 'order_form':
                $this->_title = "Создание заказа";
                $html = $this->createOrder();
                break;

            case 'show_mail_template':
                $html = $this->showMailTemplate();
                break;
            ...
            case 'general_sorting_load_more':
                $this->generalSortingLoadMore();
                break;
            ...
            // case 'get_csv':
            //     $this->catalogMgr->exportEmailsToCsv();
            //     break;
    ...
    function GetTabs()
    {
        ...
        if(App::$User->IsInRole('u_catalog_section_settings_manager')) {
    ...
    private function _PostAction()
    {
        ...
        {
            case 'ajax_calc_order_price':
                $this->_PostAjaxCalcOrderPrice();
                break;
            case 'ajax_surcharge_order':
                $this->_PostAjaxSurchargeOrder();
                break;
...
echo "OK"; exit;
    ...
    private function _GetProducts()
    {
        ...
        $parentId = App::$Request->Get['parent_id']->Int(0, Request::UNSIGNED_NUM);
        $filterName = App::$Request->Get['filter_name']->Value(Request::OUT_HTML_CLEAN );
        $filterCode = App::$Request->Get['filter_code']->Value(Request::OUT_HTML_CLEAN );
        $prevUrl = App::$Request->Get['prev_url']->Value(Request::OUT_HTML_CLEAN );
        $isFilter =  App::$Request->Get['is_filter']->Enum(0, [0, 1], Request::UNSIGNED_NUM);

        $field  = App::$Request->Get['field']->Enum('name', ['name', 'article', 'isvisible', 'isavailable', 'ord']);
        ...
        $paginationLink = '?' . http_build_query($paginationLink);

        $pages = Data::GetNavigationPagesNumber($rowsOnPage, self::$PAGES_ON_PAGE, $count, $page, $paginationLink . '&page=@p@');
        ...
        return STPL::Fetch('admin/modules/catalog/products_list', array(
            ...
            'type_id'     => $type_id,
            'section_id'  => $this->_id,
            'isFolder' => $category->Kind == CatalogMgr::CK_FOLDER,
            'parentId' => $parentId,
            'bouquetCategoryId' => 3,
            'page'        => $page,
            'sections'    => $this->catalogMgr->GetCatalog(),
    ...
    private function _GetProductEdit()
    {
        ...
        // $discount = $product->getDiscountPrice($this->_id);
        // $refs = $product->GetAreaRefs($this->_id);
        ...
        $form['ParentId'] = $product->ParentId;
        ...
        if ( App::$Request->requestMethod === Request::M_POST )
        {
            ...
            $form['IsNew']           = App::$Request->Post['IsNew']->Enum(0, array(0,1));
            $form['IsAvailable']     = App::$Request->Post['IsAvailable']->Enum(0, array(0,1));
            $form['ExcludeDiscount']     = App::$Request->Post['ExcludeDiscount']->Enum(0, array(0,1));
            $form['Discount']     = App::$Request->Post['Discount']->Int(0, Request::UNSIGNED_NUM);
            $form['ShortDesc']       = App::$Request->Post['ShortDesc']->Value(Request::OUT_HTML_CLEAN | Request::OUT_CHANGE_NL);
            $form['Text']            = App::$Request->Post['Text']->Value();
            $form['CompositionText'] = App::$Request->Post['CompositionText']->Value();
            $form['Theme']           = App::$Request->Post['Theme']->Enum(0, array_keys(CatalogMgr::$THEMES));
        ...
        else
        {
            ...
            $form['ExcludeDiscount'] = $areaRefs['ExcludeDiscount'];
            $form['Discount'] = $areaRefs['Discount'];
        }
    ...
    private function _ToggleVisibleProductType()
    {
        ...
        if(is_null($types)) {
            ...
            $filter['TypeIds'] = $product->category->Kind == CatalogMgr::CK_WEDDING ? [9] : [1,3,10,12];
    ...
    private function _GetAjaxToggleProductAvailable()
    {
        ...
        if($IsAvailable == 0) {
            $filter['TypeIds'] = $category->Kind == CatalogMgr::CK_WEDDING ? [9] : [1,3,10, 12];
            $filter['SectionID'] = $this->_id;
    ...
    private function _PostProductsOrderSave()
    {
        ...
        // $orders = App::$Request->Post['Ord']->AsArray(array(), Request::UNSIGNED_NUM);
        $orders = App::$Request->Post['order']->AsArray(array(), Request::UNSIGNED_NUM);

        if (!is_array($orders) || count($orders) == 0)
            return true;

        // foreach($orders as $productId => $ord) {
        foreach($orders as $key => $productId) {
            $ord = $key + 1;
            $product = $this->catalogMgr->GetProduct($productId);
            if ($product === null)
                continue;
    ...
    private function _PostProduct()
    {
        ...
        $ExcludeDiscount     = App::$Request->Post['ExcludeDiscount']->Enum(0, array(0,1));
        $Discount     = App::$Request->Post['Discount']->Int(0, Request::UNSIGNED_NUM);
    ...
    private function _GetCompositions()
    {
        ...
        $dir = App::$Request->Get['dir']->Enum('asc', ['asc', 'desc']);
        $article = App::$Request->Get['filter-article']->Value('');
        $filterArticle = App::$Request->Get['filter-article']->Value('');
        $filterName = App::$Request->Get['filter-name']->Value('');
        $page = App::$Request->Get['page']->Int(1, Request::UNSIGNED_NUM);
        ...
        list($members, $count) = $this->catalogMgr->GetMembers($filter);

        $pages = Data::GetNavigationPagesNumber(
             $rows, self::$PAGES_ON_PAGE, $count, $page,
             "?section_id=". $this->_id ."&action=compositions&field=".$field."&dir=".$dir."&page=@p@");
    ...
    {
        if(App::$User->IsInRole('u_catalog_section_settings_manager') == false &&
            (App::$User->IsInRole('e_adm_execute_section') && App::$User->IsInRole('e_adm_execute_users')) == false) {
            UserError::AddErrorIndexed('global', ERR_A_SECTION_ACCESS_DENIED);
            return STPL::Fetch('admin/modules/catalog/errors');
        }
    ...
    private function _PostConfigSettings()
    {
        ...
        $discount_value     = App::$Request->Post['discount_value']->Int(0, Request::UNSIGNED_NUM);
    ...
    private function generalSortingLoadMore()
    {
        include_once ENGINE_PATH.'include/json.php';
        $json = new Services_JSON();
        ini_set('memory_limit', '128M');
        ...
        $page = App::$Request->Get['page']->Int(1, Request::UNSIGNED_NUM);
        $perPage = App::$Request->Get['perPage']->Int(1, Request::UNSIGNED_NUM);
        $sectionId = App::$Request->Get['section_id']->Int(0, Request::UNSIGNED_NUM);
        $typeIds = App::$Request->Get['type_id']->AsArray([]);
        $isActual = App::$Request->Get['actual']->Enum(0, [0, 1]);

        $filter = [
           'flags' => [
               'Categories' => explode(',', $typeIds),
               'ParentId' => 0,
               'all' => true,
               'with' => array('AreaRefs'),
               'objects' => true,
        ...
        $viewSuffix = $isActual? '_preview': '';

        $json->send([
            'status' => 'ok',
            'content' => STPL::Fetch('admin/modules/catalog/_general_sorting_products' . $viewSuffix, [
                'products' => $products,
    ...
    private function _GetMailing()
    {
        // if($_GET['a']) {
        //     echo App::$Protocol; exit;
        // }

//         LibFactory::GetStatic('phpmailsender');
//         $order = $this->catalogMgr->GetOrder(6175);

//         $citiesMgr = CitiesMgr::getInstance();
//         setlocale(LC_TIME, ["ru_RU.utf-8", "ru_RU", "ru"]);
//         $day = strftime("%A");
//         $day = mb_strtoupper(mb_substr($day, 0, 1)) . mb_substr($day, 1);

//         $discountCard = $this->catalogMgr->GetDiscountCardByCode($order->discountcard);

//         $msg = STPL::Fetch('modules/catalog/mail/payment', [
//             'domain' => MAIN_DOMAIN,
//             'date' =>  $day . ' ' .  date('d.m.Y'),
//             'order' => $order,
//             'district' => $citiesMgr->GetDistrict($order->DistrictID),
//             'store' => $citiesMgr->GetStore($order->StoreID),
//             'discountCard' => $discountCard,
//             // 'discount' => $discount,
//         ]);

//         // PhpMailSender::send('dob.roid@gmail.com', 'Theme subject', $msg);
//         AdminModule::SendMail('dob.roid@gmail.com', 'Theme subject', $msg);
...
// echo "ok";
        ...
        //     $mail->Username = 'dev@aroundcity.club';                 // SMTP username
        ...
        //     $mail->addAddress('mail@gmail.com', 'Joe User');     // Add a recipient

        //     //Attachments
        //     // $mail->addAttachment('/var/tmp/file.tar.gz');         // Add attachments
        //     // $mail->addAtt`achment('/tmp/image.jpg', 'new.jpg');    // Optional name
        ...
        // $subject = 'the subject';
        // $message = 'hello';
        // $headers = 'From: noreply@rosetta.florist' . "\r\n" .
        //     'Reply-To: noreply@rosetta.florist' . "\r\n" .
    ...
    // $card_list = $this->catalogMgr->GetDiscountCards($filter);
        ...
        $field  = App::$Request->Get['field']->Enum('created', ['code', 'orderid', 'isactive', 'discount']);
   ...
   private function _GetDiscountCardEdit()
   {
        ...
        if ( App::$Request->requestMethod === Request::M_POST )
        {
            ...
            $form['discount'] = $discountcard->discount;
        }
...
// $rk = PaymentMgr::GetInstance()->GetDefaultAcc();
// $params['MNT_TRANSACTION_ID'] = '123123123';
// $xmlInventory = $rk->getXmlInventory($order, $params);
// echo $xmlInventory;
// ========================================================
...
//=========================================================
// $mailMessage = STPL::Fetch('modules/catalog/mail/surcharge', [
//     'domain' => $_SERVER['HTTP_HOST'],
//     'date' => date('d.m.Y'),
//     'sum' => number_format($order->totalprice, 0, '', ''),
//     'id' => $order->OrderID,
//     'newId' => $order->OrderId,
//     'order' => $order,
...
//        echo '<pre>'.var_export($postcards, true).'</pre>';
    ...
    private function createOrder()
    {
        ...
        $info = App::$Request->Post['order']->AsArray([]);
        if($info) {
            if(!$info['DeliveryDate']) {
                UserError::AddErrorIndexed('DeliveryDate', ERR_A_CUSTOM_ORDER_DELIVERY_DATE_EMPTY);
            }

            $composition = $this->getOrderComposition($post);
            if(count($composition) == 0) {
                UserError::AddErrorIndexed('composition', ERR_A_PRODUCT_COMPOSITION_IS_EMPTY);
            }

            $discount = App::$Request->Post['discount']->Int(0, Request::UNSIGNED_NUM);
            $time = App::$Request->Post['time']->AsArray([]);
            $info['DeliveryTime'] = $time['from'] . ' - ' . $time['to'];
            $info['PaymentDate'] = 0;
            $info['Status'] = CatalogMgr::OS_NEW;
            $info['PaymentStatus'] = CatalogMgr::PS_NOPAID;
            $info['DeliveryDate'] = strtotime($info['DeliveryDate']);
            $info['CorrectionCall'] = $info['CorrectionCall'] ? $info['CorrectionCall'] : 2;
            $info['UserID'] = App::$User->ID;
            $info['PaymentType'] = CatalogMgr::PT_ONLINE;

            if($info['DeliveryType'] == CatalogMgr::DT_DELIVERY) { // доставка
                $deliveryInfo = $this->getDistrictDeliveryInfo($info['DistrictID']);

                $info['DeliveryArea'] = $deliveryInfo['area'];
                $info['DeliveryPrice'] = $deliveryInfo['price'];

                $info['StoreID'] = 0;
            } else if($info['DeliveryType'] == CatalogMgr::DT_PICKUP) { // самовывоз
                $info['StoreAddress']    = $this->getStoreAddress($info['StoreID']);

                $info['DistrictID'] = 0;
            }

            $info['TotalPrice'] = $this->getDetailOrderPrice($info, $composition);


            if($info['CardID']) {
                $card = $this->catalogMgr->GetCard($info['CardID']);
                $info['CardName'] = $card->Name;
                $info['CardPrice'] = $card->Price;
            }

            $order = new Order($info);
            if($order->Update()) {
                $order->addCustomRef($composition);
                // $this->sendPaymentLink($order, $discount);
            ...
            $order = new Order([]);
            $order->DeliveryType = CatalogMgr::DT_DELIVERY;
            $time = [
                'from' => '',
                'to' => '',
            ];
        ...
        if($order->DeliveryDate) {
            $order->DeliveryDate = date('d.m.Y', $order->DeliveryDate);
        }

        return $this->renderOrderForm($order, $time, $discount);
    }

    private function getDistrictDeliveryInfo($districtId)
    {
        $district = CitiesMgr::GetInstance()->GetDistrict($districtId);
        if($district === null) {
            return null;
        }
    ...
    private function getStoreAddress($storeId)
    {
        $store = CitiesMgr::GetInstance()->GetStore($store_id);
        if($store == null) {
    ...
    private function _PostAjaxCalcOrderPrice()
    {
        include_once ENGINE_PATH.'include/json.php';
        $json = new Services_JSON();
        ...
        $info = $post['order'];
        $composition = $this->getOrderComposition($post);
        $totalPrice = $this->getDetailOrderPrice($info, $composition);

        if(count($composition) == 0) {
            $json->send(array(
                'status'      => 'error',
                'message'        => 'Состав заказа пустой',
            ));
            exit;
        }
        ...
        if($info['DiscountCard']) {
            $result['withDiscount'] = true;
        } else {
            $result['withDiscount'] = false;
        }
        $result['discountCardIsValid'] = $this->checkValidateDiscountCard($info['DiscountCard']);
    ...
    private function _PostAjaxSurchargeOrder()
    {
        include_once ENGINE_PATH.'include/json.php';
        $json = new Services_JSON();
        ...
        $composition = $this->getOrderComposition($post);
        $sum = $this->getDetailOrderPrice([], $composition);
        ...
        $newOrder->DeliveryPrice = 0; // убрать стоимость доставки
        $newOrder->DeliveryType = 0; // убрать стоимость доставки
        $newOrder->DistrictID = 0; // убрать стоимость доставки
        $newOrder->CardID = 0; // убрать открытку
        $newOrder->CardText = ''; // убрать открытку
        $newOrder->CardPrice = ''; // убрать открытку
        $newOrder->StoreID = null; // убрать открытку
        $newOrder->DiscountCard = ''; // Убрать скидочную карту
        $newOrder->TotalPrice = $sum;
        $newOrder->IsArchive = 0; // Убираем архивацию заказа
        ...
        $newOrder->addCustomRef($composition);
        $mailMessage = STPL::Fetch('modules/catalog/mail/surcharge', [
            'domain' => $_SERVER['HTTP_HOST'],
            'date' => date('d.m.Y'),
            'sum' => $sum,
        ...
        $subject = 'Доплата за заказ №' . $order->OrderID;

        AdminModule::SendMail($to, $subject, $mailMessage);
        ...
        // $mail->AddBody('text', $mailMessage, MailSender::BT_HTML, 'utf-8');
        // $mail->SendImmediate();
    ...
    private function getOrderComposition($post)
    {
        $composition = [];

        foreach($post['products'] as $k => $item) {
            if(!$item || !$post['counts'][$k] || !$post['prices'][$k]) {
                continue;
            }

            $product = [];
            $product['name'] = $item;
            $product['count'] = $post['counts'][$k];
            $product['price'] = $post['prices'][$k];
    ...
    private function getDetailOrderPrice($info, $products)
    {
        $productsCost = $this->getProductsCost($products);
        $cardPrice = $this->getCardPrice($info['CardID']);
        $deliveryPrice = $info['DeliveryType'] == CatalogMgr::DT_DELIVERY ? $this->getDeliveryPrice($info['DistrictID']) : 0;
        $discount = $this->getDiscountByCard($info['DiscountCard']);
        ...
        $priceWithoutDelivery = $productsCost + $cardPrice;

        // $discountedPrice = round($priceWithoutDelivery - (($priceWithoutDelivery / 100) * $discount), 1);
        $discountedPrice = floor($priceWithoutDelivery - (($priceWithoutDelivery / 100) * $discount));
    ...
    private function getDiscountByCard($discountcard)
    {
        $discount = 0;
        ...
        if($this->checkValidateDiscountCard($discountcard)) {
            $objcard = $this->catalogMgr->GetDiscountCardByCode($discountcard);
            return $objcard->discount;
        }
    ...
    private function getCardPrice($cardId)
    {
        $cardPrice = 0;
        $card = $this->catalogMgr->GetCard($cardId);
        if($card) {
    ...
    private function getDeliveryPrice($districtId)
    {
        $deliveryPrice = 0;
        $district = $this->cityMgr->GetDistrict($districtId);
        if($district) {
            $deliveryPrice = $district->Price;
        }

        return $deliveryPrice;
    }

    private function renderOrderForm($order, $time, $discount)
    {
        $citiesMgr = CitiesMgr::getInstance();
        ...
        return STPL::Fetch("admin/modules/catalog/order_form", [
            'order' => $order,
            'deliveryTypes' => CatalogMgr::$DT_TYPES,
            'delivery' => CatalogMgr::DT_DELIVERY,
            'pickup' => CatalogMgr::DT_PICKUP,
            'districts' => $citiesMgr->GetDistricts($districtsFilter),
            'stores' => $citiesMgr->GetStores($storesFilter),
            'catalogId' => $this->_id,
            'times' => $times,
            'time' => $time,
            'cards' => $this->catalogMgr->GetCards($filterCards),
            'action' => 'order_save',
            'discount' => $discount,
        ]);
    }

    private function showMailTemplate()
    {
        $order = $this->catalogMgr->GetOrder(6534);
        $discount = 250;
        $citiesMgr = CitiesMgr::getInstance();
        setlocale(LC_TIME, ["ru_RU.utf-8", "ru_RU", "ru"]);
        $day = strftime("%A");
        ...
        return STPL::Fetch('modules/catalog/mail/payment', [
            'domain' => MAIN_DOMAIN,
            'date' =>  $day . ' ' .  date('d.m.Y'),
            'order' => $order,
            'district' => $citiesMgr->GetDistrict($order->DistrictID),
    ...
    // private function sendPaymentLink($order, $discount)
    private function sendPaymentLink($order)
    {
        $citiesMgr = CitiesMgr::getInstance();
        setlocale(LC_TIME, ["ru_RU.utf-8", "ru_RU", "ru"]);
        $day = strftime("%A");
        ...
        $subject = 'Оплата заказа №' . $order->OrderID;

        AdminModule::SendMail($to, $subject, $msg);

        // LibFactory::GetStatic('mailsender');
        // $mail = new MailSender();
        // $mail->AddAddress('from', 'no-reply@rosetta.florist', "Служба уведомлений", 'utf-8');
        // $mail->AddHeader('Subject', 'Оплата заказа №' . $order->OrderID, 'utf-8');
        // $mail->body_type = MailSender::BT_HTML;
    ...
    private function _PostOrder()
    {
        ...
        if($old_status != $status) {
            error_log('adm.index: status changed. order ' . $order->id . ', prev ' . $old_status . ', new ' . $status . ', payed ' . $order->paymentstatus);
            $msg = '';
            ...
            if($status == CatalogMgr::OS_ACCEPT) {
                ...
                if($order->WantDiscountCard == 1
                    ...
                    if($config['notice']['email']) {
                        ...
                        $card_subject = "Скидочная карта в интернет-магазине Розетта ".$this->_env['site']['domain'];
                        AdminModule::SendMail($order->customeremail, $card_subject, $dcard_letter);
                    }
            ...
            if($send_mail && $config['notice']['email']) {
                ...
                AdminModule::SendMail($order->customeremail, $mail_subject, $mail_letter);
                // $mail = new MailSender();
                // $mail->AddAddress('from', 'no-reply@rosetta.florist', "Служба уведомлений", 'utf-8');
    ...
    private function _PostLoadCards()
    {
        ...
        for ($row = 1; $row <= $highestRow; ++$row)
        {
            ...
            $discount = $worksheet->getCellByColumnAndRow(2, $row);
