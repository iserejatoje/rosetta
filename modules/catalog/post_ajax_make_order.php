...
$contact_phone     = App::$Request->Post['contact_phone']->Value();
    ...
    // check for emoji and replace it if it's there
    $copy_card_text = $card_text;
    ...
    $total_price = $cart['sum']['total_price'] + $card_price;
    ...
    $total_price = $this->catalogMgr->getCartPrice($cart, $card_price, $discount);

    $total_price += $delivery_price;

    /*$price_wo_delivery = $cart['sum']['total_price'] + $card_price;
    ...
    $data = [
        ...
        'CardID'          => $card_id,
        'CardText'        => $replaced_emoji_card_text,
        'CardName'        => $card_name,
    ...
    if($card_id > 0) {
        $letter .= $card_name."<br>";
        $letter .= "Текст открытки: ".$card_text."<br>";
    } else {
        $letter .= "Без открытки<br/>";
    }

    foreach($order->refs as $item) {
        $letter .= "Артикул букета: ".$item['RealProduct']->article."<br>";
